...
func main() {
	...
	// Contar la cantidad de días de cada tipo en el rango
	conteo := contarDiasSemana(FechaInicio, FechaFin)

	// Imprimir el conteo de días de la semana
	fmt.Println("\nCantidad de cada día de la semana:")
	for dia, cantidad := range conteo {
		fmt.Printf("%s: %d\n", dia, cantidad)
	}
}
